...
function getSQLConnection()
{
	...
	if($_mysqli -> connect_error)
	{
		fatalError("BIONAL was unable to connect to the MySQL database. The database has either gone offline/unreachable, or BIONAL is not configured properly. Please contact the server administrator.<br><br>" . $_mysqli -> connect_error);
		return false;
	}
...
function querySQL($query)
{
	...
	if($result === false)
	{
		...
		fatalError("BIONAL encountered an SQL query error. This is most likely a bug in Agora, please report this occurence; but make sure that the data below doesn't contain any sensitive information (like your password hash). If it does, censor it before reporting.<br><br>Technical details:<br>\nError: " . $mysqli -> error . " \n<br>\nSource function: " . debug_backtrace()[1]['function'] . "\n<br>\nFull query: " . $query);
		return false;
	}
